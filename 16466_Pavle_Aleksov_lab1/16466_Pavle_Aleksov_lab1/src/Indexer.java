...
public class Indexer extends BaseConfig {
	...
	public void createIndex() {
		...
		IndexWriterConfig config2 = new IndexWriterConfig(this.analyzer);
		config2.setOpenMode(OpenMode.CREATE);
		
		try (Directory indexDir = FSDirectory.open(Paths.get(DIREKTORIJUM_SA_INDEKSOM_PODELJENI));
				IndexWriter indexWriter = new IndexWriter(indexDir, config2);) {
			
			File splitDataDir = new File(DIREKTORIJUM_SA_PODELJENIM_PODACIMA);
			
			long start = System.currentTimeMillis();
			addDataToIndex(splitDataDir, indexWriter);
			long end = System.currentTimeMillis();
			System.out.println("Indexing split files took "
				      + (end - start) + " milliseconds");
