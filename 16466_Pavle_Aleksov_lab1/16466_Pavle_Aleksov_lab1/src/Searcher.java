...
public class Searcher extends BaseConfig implements AutoCloseable {
	...
	public void logickiUpit() {
		// query parser
		parseClassicAndSearch("romeo OR Mowgli AND NOT Gulliver");

		// query object
		BooleanQuery.Builder booleanQueryBuilder = new BooleanQuery.Builder();
		TermQuery termRomeo = new TermQuery(new Term(POLJE_SADRZAJ, "romeo"));
		TermQuery termMowgli = new TermQuery(new Term(POLJE_SADRZAJ, "mowgli"));
		TermQuery termGulliver = new TermQuery(new Term(POLJE_SADRZAJ, "gulliver"));
		booleanQueryBuilder = new BooleanQuery.Builder();
		booleanQueryBuilder.add(termRomeo, Occur.SHOULD);
		booleanQueryBuilder.add(termMowgli, Occur.MUST);
	...
	public void pointRangeUpit() {
		// query parser
		parsePointAndSearch(POLJE_VELICINA_LONG + ":{139264 TO 170000]", POLJE_VELICINA_LONG);
		
		// query object
		Query pointRangeQuery1 = LongPoint.newRangeQuery(POLJE_VELICINA_LONG, 139264, 170000);
